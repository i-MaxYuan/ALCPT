...
class UserList(View,OnlineUserStat):
    ...
    def do_content_works(self,request):
        ...
        if keywords['name'] and any(char in punctuation for char in keywords['name']):  #punctuation: !"#$%&'()*+,-./:;<=>?@[\]^_`{|}~
            keywords['name'] = None
        ...
        dicts= dict(departments = Department.objects.all(),
...
# 更改使用者
@method_decorator(permission_check(UserType.SystemManager),name='get')
class UserEdit(View,OnlineUserStat):
    ...
    def do_content_works(self,request,reg_id):
        try:
            ...
            return dict(edited_user = edited_user,
                        privileges = self.privileges,
                        identities = self.identities,
...
class UnitEdit(View,OnlineUserStat):
    ...
    def do_content_works(self,request,unit_kind,unit_name):
        if unit_kind == 'squadron':
            return dict(edited_unit = Squadron.objects.get(name=unit_name))
        
        elif unit_kind == 'department':
            return dict(edited_unit = Department.objects.get(name=unit_name))
...
class UnitMemberList(View,OnlineUserStat):
    ...
    def do_content_works(self,request,unit_kind,unit_name):
        if unit_kind == 'squadron':
            ...
            try:
                return dict(viewed_unit = viewed_unit,
...
class ReportReply(View,OnlineUserStat,FromWhere):
    ...
    def post(self,request,report_id):
        ...
        back_url = FromWhere().from_where('ResponsibleReportList') #取得ResponsibileReportList執行時的url
...
class ViewProfile(View, OnlineUserStat):
    ...
    def do_content_works(self,request,reg_id):
        try:
            return dict(viewed_user = User.objects.get(reg_id=reg_id),
                    privileges = UserType.__members__)
